...
def prepare_activity(
    ...
    *,
    ...
    if preparer_fn is None:
        ...
        # C-06 (VAREX-004): pass the parent's variable->setter map so @variables('X') inside the ForEach
        # body routes through {{tasks.X.values.Y}} instead of an undeclared {{job.parameters.X}}.
        prepared = preparer_fn(activity, scope=scope, variable_task_keys=variable_task_keys)
    else:
...
def prepare_workflow(pipeline: Pipeline) -> PreparedWorkflow:
    ...
    for activity in pipeline.tasks:
        ...
        # C-04 (NB-ITER2-4 / LSC2-001): walk into IfCondition/Switch/ForEach branches so cluster_hints
        # picks up nested cluster config; else the default cluster ships even when an inner notebook has an LS.
    ...
    # VAREX3-003: flag a sibling read of a variable mutated only inside a ForEach inner job -- the post-ForEach
    # "latest value" is unreachable across the run_job_task boundary in DAB, so emit a manual_variable_rollup.
    ...
    # C-36 (SCHED4-001): the periodic primitive can't encode hours/minutes/weekDays, so emit a
    # manual_schedule_time_of_day SetupTask for the user to add the time-of-day to the cron expression.
...
def _detect_cross_foreach_variable_reads(activities: list[Activity]) -> list[SetupTask]:
    ...
    # Variables also set OUTSIDE any ForEach are safe -- the parent always has a fresh setter to point at.
    ...
    def _read_refs(text: str) -> set[str]:
        if not isinstance(text, str):
            return set()
        return {match.group(1) for match in var_ref_pattern.finditer(text)}
