...
# write_bundle call and surfaced via the bundle's ``variables`` block + SETUP.md.
_cross_bundle_variables: dict[str, str] = {}

# C-43 (CF5-001 / CF5-002): condition_task operands the dangling-ref safety net blanked
# ({task_key, field, original_ref}). Reset per write_bundle call and surfaced in SETUP.md so a
...
# existing notebook (absolute workspace path), flowx can't inject the runtime computation, so these
# end up as manual work for the user.
...
def _extract_manual_parameters_from_existing_notebook_tasks(
    ...
    """Finds base_parameters flowx couldn't evaluate for notebook tasks.
...
def _bind_cluster_to_notebook_tasks(tasks: list[dict[str, Any]]) -> None:
    ...
    for task in _iter_tasks_recursively(tasks):
        ...
        if compute_mode == "serverless":
            # Serverless cannot host jar/whl libraries, so when the task ships libraries we still bind a
            # classic cluster so the Jobs API accepts the libraries block.
        ...
        if cluster_key is not None:
            ...
            continue
        notebook_path = notebook_task.get("notebook_path", "")
        # Stub notebooks (../src/...) are normally left unbound for serverless compute, but when libraries
        # are attached we must bind a real cluster (NB-2) -- serverless cannot install jar/whl libraries.
...
def _strip_dangling_task_value_refs(
    ...
    def visit(task: dict[str, Any]) -> None:
        ...
        # C-12: condition_task operands can also carry dangling refs when an upstream renamed task
        # disappeared between rewrite passes. C-43: record each neutralised operand for SETUP.md.
...
def _build_job_resource(
    ...
    *,
    ...
    if schedule_spec:
        ...
        # SCHED3-003: trigger-supplied parameter overrides update the matching job.parameter defaults so
        # scheduled runs get the trigger's pinned values; unknown names are ignored to keep job_def valid.
...
def _load_report(report_path: Path) -> list[PreparedWorkflow]:
    ...
    if "translations" in report:
        ...
        # Group by pipeline and route each group through _pipeline_dict_to_workflow (same machinery as the
        # single-pipeline IR format) so secret discovery / setup tasks / control-flow handling all match.
        ...
        for translation in report.get("translations", []):
            ...
            # Round-trip pipeline-level parameters supplied per-translation (newer shape) or under
            # ir.parameters (older single-pipeline serialisations roundtripped through this aggregator).
            ...
            # Likewise carry pipeline-level schedule through to the rehydrated pipeline_dict so
            # trigger-derived schedule/trigger blocks survive the aggregated report shape.
...
def pipeline_dict_to_ir(pipeline_dict: dict[str, Any]) -> tuple[Pipeline, list[dict[str, Any]]]:
    ...
    for param in pipeline_dict.get("parameters") or []:
        ...
        if "default" in param and param["default"] is not None:
            ...
            # Bool / int / float defaults must survive the JSON round-trip as their declared type so the
            # YAML carries a real boolean/number; string defaults go through normalize_value for ADF refs.
            if isinstance(default_value, bool):
                entry["default"] = default_value
            elif isinstance(default_value, (int, float)):
                entry["default"] = default_value
            elif isinstance(default_value, (list, dict)):
                # Array / Object defaults are JSON-encoded to a string at emission time; keep the structure
                # here so both bundle paths converge (a Python str() here would emit invalid JSON).
                entry["default"] = default_value
            else:
...
def _reconstruct_configuration(raw: dict[str, Any] | None) -> Any:
    ...
    if not raw:
        return None
    from flowx.adapter.models import TranslationConfiguration

    # Reports authored before the databricks_task_compute option was removed may still carry that key;
    # drop it silently so old reports remain rehydratable.
    ...
    if task_type == "IfConditionActivity":
        return IfConditionActivity(
            **base,
            ...
            # C-14 (CF3-001 / VAREX3-001): preserve bridge fields so the preparer can re-synthesise the
            # hidden _bridge SetVariable task after a JSON roundtrip.
    ...
    if task_type == "SwitchActivity":
        return SwitchActivity(
            **base,
            ...
            # C-14 (CF3-001 / VAREX3-001): preserve bridge fields for Switch so the preparer can
            # re-synthesise the bridge task after a JSON roundtrip.
            bridge_notebook_code=task_ir.get("bridge_notebook_code"),
            bridge_notebook_imports=list(task_ir.get("bridge_notebook_imports") or []),
            bridge_required_parameters=dict(task_ir.get("bridge_required_parameters") or {}),
        )
